...
try:
    from scikits.umfpack import spsolve as _spsolve  # noqa: F401
except Exception:  # pragma: no cover - fallback when scikit-umfpack is unavailable
...
def newton(
    ...
    for iter in range(maxiter):
        ...
        with warnings.catch_warnings():
            ...
            delta = _spsolve(scale @ jac, scale @ -fun_val)
...
def dogleg(fun_val, jac, radius, precond, debug) -> ndarray:
    ...
    if norm(p) == radius:
        ...
        with warnings.catch_warnings():
            if not debug:
                warnings.simplefilter("ignore")
            z: ndarray = _spsolve(scale @ jac, scale @ -fun_val)
