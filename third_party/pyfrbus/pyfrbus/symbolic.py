...
def take_symengine_partial(eq: Expr, w_resp_to: Symbol, data_hash: Dict[str, str]):
    ...
    # SymEngine fails to recognize derivative of Max/Min/Abs/Piecewise as
    # Heaviside/sign, returning an unevaluated Derivative(...). Fail over to
    # SymPy. NB: sympy.diff must receive a *native* sympy expression, so we
    # convert the symengine expression with ._sympy_() first (passing the
    # symengine expression directly makes sympy treat max/min as unknown
    # functions and it returns Derivative unchanged).
    if re.findall("Derivative", deriv):
        sy_eq = eq._sympy_()
        sy_var = sympy.Symbol(str(w_resp_to))
        deriv = str(sympy.diff(sy_eq, sy_var))
        # Once more: declare symbols real so Abs differentiates to sign(...)
        # rather than a re()/im() Derivative expression.
        if re.findall("Derivative", deriv):
            reps = {s: sympy.Symbol(s.name, real=True) for s in sy_eq.free_symbols}
            sy_eq_real = sy_eq.xreplace(reps)
            sy_var_real = sympy.Symbol(str(w_resp_to), real=True)
            deriv = str(sympy.diff(sy_eq_real, sy_var_real))
