...
current Python 3.11 / Streamlit Community Cloud runtime. Four **surgical,
...
| `pyfrbus/stochsim.py` | `index_mat.iteritems()` → `index_mat.items()`. | `DataFrame.iteritems()` was removed in pandas ≥ 2.0; `.items()` is the identical replacement. Only affects `stochsim` (the fan-chart engine). |
